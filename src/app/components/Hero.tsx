  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] text-center relative overflow-hidden">
      ...
      <div className="px-4 animate-in fade-in duration-700 z-10 mt-10 md:mt-0 pb-32"> {/* pb-32 eklendi: İçerik footer'ın altında kalmasın diye */}
        <div className="bg-brand/10 text-brand px-4 py-1.5 rounded-full text-sm font-bold mb-6 border border-brand/20 inline-flex items-center gap-2 animate-pulse">
          <Sparkles size={14} /> Yapay Zeka Destekli Ajans Deneyimi
        </div>
        <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 tracking-tight leading-tight">
          Markanızı <span className="text-transparent bg-clip-text bg-linear-to-r from-brand to-red-500">Geleceğe</span><br/>
          Taşıyoruz.
        </h1>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 mb-20">
          <button 
            ...
            className="bg-brand hover:bg-brand-900 text-white px-8 py-4 rounded-xl font-bold text-lg transition-all transform hover:scale-105 shadow-[0_0_30px_rgba(128,0,0,0.4)] flex items-center justify-center gap-2"
          >
      ...
      <div className="w-full bg-black/60 backdrop-blur-xl border-t border-white/5 pt-6 pb-4 absolute bottom-0 left-0 z-20 flex flex-col gap-4">
        ...
        <div className="absolute inset-y-0 left-0 w-20 bg-linear-to-r from-black to-transparent z-10 pointer-events-none"></div>
        <div className="absolute inset-y-0 right-0 w-20 bg-linear-to-l from-black to-transparent z-10 pointer-events-none"></div>
          ...
          <div className="flex animate-infinite-scroll gap-16 min-w-full shrink-0 items-center justify-around px-8">
            ...
            {REFERANSLAR.map((ref, i) => (
              <div key={i} className="flex items-center gap-2 opacity-50 hover:opacity-100 transition-opacity duration-300 group cursor-default">
                <ref.icon className="w-6 h-6 text-brand group-hover:scale-110 transition-transform" />
